...
interface CoingeckoRepository {
    ...
    fun getChartData(interval: ChartIntervalType, currencyCode: String, coin: String): Result<List<ChartDataModel>>

    fun getHistoryData(currencyCode: String, from: Long, to: Long, coin: String): Result<List<MarketHistoryDataModel>>
    ...
    private class Impl(
        ...
        override fun getHistoryData(currencyCode: String, from: Long, to: Long, coin: String): Result<List<MarketHistoryDataModel>> {
            try {
                val history = api.getHistoryData(currency = currencyCode.lowercase(), from = from, to = to, apiKey = apiKey, id = coin).execute().body()
        ...
        override fun getChartData(interval: ChartIntervalType, currencyCode: String, coin: String): Result<List<ChartDataModel>> {
            try {
                var data: ChartDataResponse? = null

                when(interval) {
                    ChartIntervalType.INTERVAL_1DAY -> {
                        data = api.get1DayChartData(currencyCode = currencyCode, apiKey = apiKey, id = coin).execute().body()!!
                    }
                    ChartIntervalType.INTERVAL_1WEEK -> {
                        data = api.get7DayChartData(currencyCode = currencyCode, apiKey = apiKey, id = coin).execute().body()!!
                    }
                    ChartIntervalType.INTERVAL_1MONTH -> {
                        data = api.get30DayChartData(currencyCode = currencyCode, apiKey = apiKey, id = coin).execute().body()!!
                    }
                    ChartIntervalType.INTERVAL_3MONTHS -> {
                        data = api.get90DayChartData(currencyCode = currencyCode, apiKey = apiKey, id = coin).execute().body()!!
                    }
                    ChartIntervalType.INTERVAL_6MONTHS -> {
                        data = api.get6MonthChartData(currencyCode = currencyCode, apiKey = apiKey, id = coin).execute().body()!!
                    }
                    ChartIntervalType.INTERVAL_1YEAR -> {
                        data = api.get1YearChartData(currencyCode = currencyCode, apiKey = apiKey, id = coin).execute().body()!!
                    }
                    ChartIntervalType.INTERVAL_ALL_TIME -> {
                        data = api.getMaxChartData(currencyCode = currencyCode, apiKey = apiKey, id = coin).execute().body()!!
                    }
                }
