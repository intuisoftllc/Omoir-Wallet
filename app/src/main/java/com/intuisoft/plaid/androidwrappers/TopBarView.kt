...
class TopBarView(context: Context, attrs: AttributeSet?) : LinearLayout(context, attrs) {
    ...
    fun setOnActionLeftClick(onClick: () -> Unit) {
        left_action_iv?.setOnSingleClickListener(Constants.Time.MIN_CLICK_INTERVAL_MED) {
            onClick()
        }
    }

    fun setOnActionRightClick(onClick: () -> Unit) {
        right_action_iv?.setOnSingleClickListener(Constants.Time.MIN_CLICK_INTERVAL_MED) {
            onClick()
        }
    }

    fun setSecondaryTextOnClick(onClick: () -> Unit) {
        secondary_text_tv?.setOnSingleClickListener(Constants.Time.MIN_CLICK_INTERVAL_MED) {
            onClick()
        }
    }
}
