...
class AtpStatusWorker(
    ...
    val firstTime = true
    val channel = "atp_channel"
    private val notificationId = 100
    var lastBatch: BatchDataModel? = null
    private val mBuilder = NotificationCompat.Builder(context,  channel)
    ...
    override suspend fun doWork(): Result {
        ...
        localStoreRepository.getAllAssetTransfers(walletId)
            .find { it.id == transferId }?.let { transfer ->
                val converter = RateConverter(0.0).setLocalRate(RateConverter.RateType.SATOSHI_RATE, transfer.expectedAmount.toDouble())
                ...
                while (transfer.status <= AssetTransferStatus.IN_PROGRESS) {
                    Thread.sleep(5000)
                    val batches = localStoreRepository.getBatchDataForTransfer(transferId)
                    val finishedBatches = batches.count { it.status >= AssetTransferStatus.PARTIALLY_COMPLETED }
                    val currentBatch = batches.findLast { it.status == AssetTransferStatus.WAITING || it.status == AssetTransferStatus.IN_PROGRESS }
                    ...
                    if(currentBatch != null && currentBatch != lastBatch && sendWallet != null) {
                        ...
                        if(currentBatch.status == AssetTransferStatus.WAITING) {
                            mBuilder.setContentText(
                                ...
                                .setProgress(batches.size, finishedBatches, true)
                        } else {
                            mBuilder.setContentText(
                                context.getString(
                                ...
                                .setProgress(batches.size, finishedBatches, false)
                        }
